...
import { correlationStrength, linearRegression, metricBucket, pearson } from '../lib/insights';

const num = (value: unknown) => {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : 0;
};
const sectionTitle = 'text-[10px] tracking-[0.2em] uppercase text-text-tertiary ml-1';
const cardClass = 'bg-bg-secondary border border-bg-tertiary rounded-2xl p-5';

const ChartTooltip = ({ active, payload, label }: any) => active && payload?.length ? (
  <div className="bg-bg-primary border border-bg-tertiary rounded-xl px-3 py-2 text-xs shadow-xl">
    <div className="text-text-tertiary">{label}</div>
    <div className="mt-1 text-text-primary">{payload[0].value}</div>
  </div>
) : null;
...
export default function Analytics() {
  ...
  const workout = (log: any) => Boolean(log.workoutCategory || (Array.isArray(log.exercises) && log.exercises.some((exercise: any) => exercise?.name?.trim())));
  ...
  const weightDomain: [number, number] = weightValues.length
    ...
    : [0, 100];
  const moodData = moods.map(item => ({ date: format(parseISO(item.date), 'MMM d'), mood: item.score, label: moodLabel(item.mood) }));
  ...
  const chart = (data: any[], key: string, color: string, domain?: [number, number]) => (
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={data} margin={{ top: 15, right: 8, left: -20, bottom: 0 }}>
        <CartesianGrid stroke="#1f2937" /><XAxis dataKey="date" stroke="#4b5563" fontSize={9} tickLine={false} axisLine={false} tickMargin={10} minTickGap={30} /><YAxis domain={domain} stroke="#4b5563" fontSize={9} tickLine={false} axisLine={false} tickMargin={8} /><Tooltip content={<ChartTooltip />} /><Line type="monotone" dataKey={key} stroke={color} strokeWidth={1.8} dot={false} activeDot={{ r: 4, fill: color }} />
      </LineChart>
    </ResponsiveContainer>
  );
  const card = (label: string, value: string | number, detail?: string) => <div className={cardClass}><div className="text-[10px] tracking-widest uppercase text-text-tertiary mb-2">{label}</div><div className="text-3xl font-light text-text-primary">{value}</div>{detail && <div className="mt-1 text-[10px] text-text-tertiary">{detail}</div>}</div>;
  const itemList = (items: any[], empty: string) => items.length ? <div className="space-y-2">{items.slice(0, 8).map(item => <div key={item.id} className="rounded-xl bg-bg-primary/40 px-3 py-3"><div className="flex items-center justify-between gap-3"><span className="text-sm text-text-primary">{item.title}</span><span className="text-[10px] text-text-tertiary">{item.enjoyment}/5</span></div><div className="mt-1 text-[9px] tracking-widest uppercase text-text-tertiary">{item.category} · {item.status}</div></div>)}</div> : <p className="text-xs text-text-tertiary">{empty}</p>;

  return (
    <div className="pb-24"><div className="max-w-6xl mx-auto p-5 sm:p-6 space-y-9">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-2">{(['Health', 'Study', 'Work'] as const).map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={`rounded-xl px-4 py-2 text-[10px] tracking-[0.16em] uppercase transition-colors ${activeTab === tab ? 'bg-bg-secondary text-text-primary border border-bg-tertiary' : 'text-text-tertiary hover:text-text-primary'}`}>{tab}</button>)}</div><div className="flex w-fit gap-1 rounded-xl border border-bg-tertiary bg-bg-secondary p-1">{[{ label: '7D', value: 7 }, { label: '30D', value: 30 }, { label: 'ALL', value: 365 }].map(item => <button key={item.value} onClick={() => setTimeRange(item.value as 7 | 30 | 365)} className={`rounded-lg px-3 py-1.5 text-[10px] tracking-widest uppercase ${timeRange === item.value ? 'bg-bg-tertiary text-text-primary' : 'text-text-tertiary'}`}>{item.label}</button>)}</div></div>
      {loading ? <div className="py-20 text-center text-xs tracking-widest uppercase text-text-tertiary animate-pulse">Loading analytics...</div> : activeTab === 'Health' ? <div className="space-y-10">
        ...
        <section><h2 className={sectionTitle}>Weight Trend</h2><div className="mt-3 h-60">{weightData.length ? chart(weightData, 'weight', '#fcd34d', weightDomain) : <div className="py-10 text-center text-xs text-text-tertiary">No weight data.</div>}</div></section>
        <section><h2 className={sectionTitle}>Mood Journey</h2><div className="mt-3 h-60">{moodData.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={moodData} margin={{ top: 15, right: 8, left: -20, bottom: 0 }}><CartesianGrid stroke="#1f2937" /><XAxis dataKey="date" stroke="#4b5563" fontSize={9} tickLine={false} axisLine={false} tickMargin={10} minTickGap={30} /><YAxis domain={[1, 5]} ticks={[1, 2, 3, 4, 5]} tickFormatter={() => ''} stroke="#4b5563" fontSize={9} tickLine={false} axisLine={false} /><Tooltip content={<ChartTooltip />} /><Line type="monotone" dataKey="mood" stroke="#5a9e8f" strokeWidth={1.8} dot={{ r: 2, fill: '#fcd34d', stroke: 'none' }} activeDot={{ r: 4, fill: '#5a9e8f' }} /></LineChart></ResponsiveContainer> : <div className="py-10 text-center text-xs text-text-tertiary">No mood data.</div>}</div></section>
        ...
        <section><h2 className={sectionTitle}>Goals Completion</h2><div className="mt-3 h-56">{goalData.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={goalData} margin={{ top: 15, right: 8, left: -20, bottom: 0 }}><CartesianGrid stroke="#1f2937" /><XAxis dataKey="date" stroke="#4b5563" fontSize={9} tickLine={false} axisLine={false} tickMargin={10} minTickGap={30} /><YAxis domain={[0, 100]} stroke="#4b5563" fontSize={9} tickLine={false} axisLine={false} /><Tooltip content={<ChartTooltip />} /><Bar dataKey="completion" fill="#c8925a" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer> : <div className="py-10 text-center text-xs text-text-tertiary">No goals logged in this range.</div>}</div></section>
        <section><h2 className={sectionTitle}>Monthly Activity & Mood Distribution</h2><div className="mt-3 grid gap-4 md:grid-cols-2"><div className={`${cardClass} h-72`}><h3 className={sectionTitle}>Monthly Activity</h3>{monthlyActivity.length ? <ResponsiveContainer width="100%" height="90%"><BarChart data={monthlyActivity} margin={{ top: 15, right: 8, left: -20, bottom: 0 }}><CartesianGrid stroke="#1f2937" /><XAxis dataKey="name" stroke="#4b5563" fontSize={9} tickLine={false} axisLine={false} /><YAxis stroke="#4b5563" fontSize={9} tickLine={false} axisLine={false} allowDecimals={false} /><Tooltip content={<ChartTooltip />} /><Bar dataKey="count" fill="#5a9e8f" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer> : <div className="py-10 text-center text-xs text-text-tertiary">No workouts in this range.</div>}</div><div className={`${cardClass} flex h-72 items-center gap-4`}><div className="h-full w-1/2">{moodDistribution.length ? <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={moodDistribution} innerRadius="58%" outerRadius="78%" dataKey="value" paddingAngle={2} stroke="none">{moodDistribution.map(item => <Cell key={item.name} fill={item.color} />)}</Pie></PieChart></ResponsiveContainer> : null}</div><div className="w-1/2 space-y-3">{moodDistribution.length ? moodDistribution.map(item => <div key={item.name} className="flex items-center justify-between gap-2 text-xs"><span className="flex items-center gap-2 text-text-secondary"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: item.color }} />{item.name}</span><span className="font-mono text-text-tertiary">{Math.round(item.value / moods.length * 100)}%</span></div>) : <span className="text-xs text-text-tertiary">No mood data.</span>}</div></div></div></section>
        <section><h2 className={sectionTitle}>Correlations</h2><div className="mt-3 grid gap-3 md:grid-cols-3">{correlations.length ? correlations.map(item => <div key={item.name} className={`${cardClass} space-y-3`}><div className="text-[10px] tracking-widest uppercase text-text-tertiary">{item.name}</div><div className="flex items-center gap-2"><span className="rounded-full bg-accent-teal/10 px-2.5 py-1 font-mono text-sm text-accent-teal">r = {item.value.toFixed(2)}</span><span className="text-xs text-text-secondary">{item.strength}</span></div><p className="text-xs leading-relaxed text-text-secondary">{item.sentence}</p></div>) : <p className="text-xs text-text-tertiary">Not enough aligned data for correlations.</p>}</div></section>
        <section><h2 className={sectionTitle}>Weight Projection</h2><div className="mt-3 grid gap-3 md:grid-cols-2"><div className={`${cardClass} h-64 md:col-span-2`}>{projectionData.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={projectionData} margin={{ top: 15, right: 8, left: -20, bottom: 0 }}><CartesianGrid stroke="#1f2937" /><XAxis dataKey="date" stroke="#4b5563" fontSize={9} tickLine={false} axisLine={false} minTickGap={30} /><YAxis domain={weightDomain} stroke="#4b5563" fontSize={9} tickLine={false} axisLine={false} /><Tooltip content={<ChartTooltip />} /><Line name="Actual" dataKey="actual" stroke="#fcd34d" strokeWidth={2} dot={false} activeDot={{ r: 4, fill: '#fcd34d' }} /><Line name="Projected" dataKey="projected" stroke="#5a9e8f" strokeWidth={1.8} strokeDasharray="5 5" dot={false} activeDot={{ r: 4, fill: '#5a9e8f' }} /></LineChart></ResponsiveContainer> : <div className="py-10 text-center text-xs text-text-tertiary">Not enough weight data.</div>}</div>{card('+7 days', projections.length ? `${projections[0].toFixed(1)} kg` : '--')}{card('+14 days', projections.length ? `${projections[1].toFixed(1)} kg` : '--')}</div><div className="mt-3 text-sm text-text-secondary">Current trend: <span className="text-text-primary">{trendLabel}</span>.</div><p className="mt-1 text-xs text-text-tertiary">Based on linear regression across all recorded weights. Assumes habits remain constant.</p></section>
